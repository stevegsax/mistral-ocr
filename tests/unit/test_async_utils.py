...
class TestConcurrentJobProcessor:
    ...
    async def test_refresh_job_statuses_async_filters_correctly(self, processor):
        ...
        result = await processor.refresh_job_statuses_async(mock_job_manager, jobs, skip_statuses)
    ...
    async def test_refresh_job_statuses_async_skips_final_statuses(self, processor):
        ...
        jobs = [
            JobInfo(
                id="job1", status="SUCCESS", submitted="2024-01-01T10:00:00Z"
            ),  # Should be skipped
            JobInfo(
                id="job2", status="running", submitted="2024-01-01T10:00:00Z"
            ),  # Should be refreshed
            JobInfo(
                id="job3", status="FAILED", submitted="2024-01-01T10:00:00Z"
            ),  # Should be skipped
        ]
    ...
    async def test_download_multiple_results_async_handles_exceptions(self, processor):
        ...
        results = await processor.download_multiple_results_async(mock_result_manager, job_ids)
...
class TestIntegrationScenarios:
    ...
    async def test_concurrent_job_processing_realistic_scenario(self):
        ...
        start_time = time.time()
        result = await processor.refresh_job_statuses_async(job_manager, jobs, skip_statuses)
        end_time = time.time()
