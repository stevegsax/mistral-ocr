...
class TestValidateModelName:
    ...
    def test_empty_model_name_raises_error(self):
        ...
        with pytest.raises(
            InvalidConfigurationError, match="Model name must be a non-empty string"
        ):
            dummy_method(None, "")
...
class TestValidateTimeoutRange:
    ...
    def test_zero_timeout_raises_error(self):
        ...
        with pytest.raises(
            InvalidConfigurationError, match="Timeout must be an integer between 1 and 3600 seconds"
        ):
            dummy_method(None, 0)
...
class TestValidateRetryCount:
    ...
    def test_negative_retries_raises_error(self):
        ...
        with pytest.raises(
            InvalidConfigurationError, match="Max retries must be an integer between 0 and 10"
        ):
            dummy_method(None, -1)
