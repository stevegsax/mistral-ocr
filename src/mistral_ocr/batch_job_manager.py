...
from .async_utils import ConcurrentJobProcessor
...
class BatchJobManager:
    ...
    def _api_get_job_status(self, job_id: str):
        ...
        try:
            ...
            # Convert network/connection errors to retryable errors
            if self._is_transient_error(e):
                raise RetryableError(f"Transient error getting job status: {e}", original_error=e)
            else:
                # Re-raise permanent errors without retry
                raise
    ...
    def _api_cancel_job(self, job_id: str):
        ...
        """
        try:
            return self.client.batch.jobs.cancel(job_id=job_id)
        except Exception as e:
            if self._is_transient_error(e):
                raise RetryableError(f"Transient error cancelling job: {e}", original_error=e)
            else:
                raise
    ...
    def _api_list_jobs(self):
        ...
        try:
            ...
            if self._is_transient_error(e):
                raise RetryableError(f"Transient error listing jobs: {e}", original_error=e)
            else:
                raise
    ...
    def check_job_status(self, job_id: str) -> str:
        ...
        try:
            batch_job = self._api_get_job_status(job_id)
            status = (
            ...
            try:
                ...
                api_response: APIJobResponse = {
                    "id": batch_job.id,
                    "status": status,
                    "created_at": str(getattr(batch_job, "created_at", None))
                    if getattr(batch_job, "created_at", None)
                    else None,
                    "completed_at": str(getattr(batch_job, "completed_at", None))
                    if getattr(batch_job, "completed_at", None)
                    else None,
    ...
    def cancel_job(self, job_id: str) -> bool:
        ...
        try:
            cancelled_job = self._api_cancel_job(job_id)
            status = (
        ...
        try:
            ...
            for api_job in api_jobs.data:
                ...
                if job_id not in local_job_ids:
                    ...
                    placeholder_doc_uuid = SERVER_JOB_DOC_TEMPLATE.format(
                        job_prefix=job_id[:UUID_PREFIX_LENGTH]
                    )
                    placeholder_doc_name = SERVER_JOB_NAME_TEMPLATE.format(
                        job_prefix=job_id[:UUID_PREFIX_LENGTH]
                    )
                    ...
                    api_response: APIJobResponse = {
                        ...
                        "completed_at": str(getattr(api_job, "completed_at", None))
                        if getattr(api_job, "completed_at", None)
                        else None,
                    ...
                    self.database.store_job_full_api_data(
                        job_id, placeholder_doc_uuid, api_response
                    )
                    synced_count += 1
    ...
    def _sync_and_refresh_all_jobs(self, local_jobs: List[JobInfo]) -> List[JobInfo]:
        ...
        try:
            ...
            for api_job in api_jobs.data:
                ...
                # Handle mock objects by safely extracting values
                def safe_getattr(obj, attr, default=None):
                    """Safely get attribute, handling Mock objects."""
                    try:
                        value = getattr(obj, attr, default)
                        # Handle Mock objects that can't be JSON serialized
                ...
                api_response: APIJobResponse = {
                    "id": job_id,
                    "status": api_status,
                    "created_at": str(safe_getattr(api_job, "created_at"))
                    if safe_getattr(api_job, "created_at")
                    else None,
                    "completed_at": str(safe_getattr(api_job, "completed_at"))
                    if safe_getattr(api_job, "completed_at")
                    else None,
                ...
                if job_id in local_job_map:
                    ...
                    if local_job.status != api_status:
                        self.logger.debug(
                            f"Job {job_id} status changed: {local_job.status} -> {api_status}"
                        )
                        # Update database with new status and API data
                        try:
                            self.database.update_job_full_api_data(job_id, api_response)
                        except Exception as e:
                            self.logger.warning(f"Failed to update database for job {job_id}: {e}")
                        ...
                        local_job.file_count = (
                            api_response.get("total_requests") or local_job.file_count
                        )
                        updated_count += 1
                    else:
                        ...
                        try:
                            ...
                            self.logger.warning(
                                f"Failed to update database refresh timestamp for job {job_id}: {e}"
                            )
                        local_job.last_api_refresh = api_response["refresh_timestamp"]
                else:
