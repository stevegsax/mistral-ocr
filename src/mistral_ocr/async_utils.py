...
class AsyncAPIManager:
    ...
    async def run_concurrent_operations(self, operations: List[Callable[[], Any]], 
    ...
    async def run_concurrent_with_progress(self, operations: List[Callable[[], Any]], 
...
def async_method_wrapper(max_concurrent: int = 10):
    ...
    def decorator(func: F) -> F:
        ...
        async def async_batch_method(self, items: List[Any], progress_callback: Optional[Callable[[int, int], None]] = None) -> List[Any]:
            ...
            if progress_callback:
                return await self._async_manager.run_concurrent_with_progress(operations, progress_callback)
            else:
...
class ConcurrentJobProcessor:
    ...
    async def refresh_job_statuses_async(self, job_manager, jobs: List[dict], 
                                       skip_statuses: set) -> List[dict]:
    ...
    async def download_multiple_results_async(self, result_manager, job_ids: List[str], 
                                            destination: Optional[Any] = None) -> List[Union[str, Exception]]:
