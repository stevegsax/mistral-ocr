...
class FileSystemUtils:
    ...
    def safe_delete_file(
        path: pathlib.Path, logger: Optional[structlog.stdlib.BoundLogger] = None
    ) -> bool:
    ...
    def check_file_size(path: pathlib.Path, max_size: Optional[int] = None) -> int:
        ...
        if max_size is not None and size > max_size:
            raise ValueError(
                f"File {path.name} size ({size} bytes) exceeds limit ({max_size} bytes)"
            )
...
class FileTypeUtils:
    ...
    def filter_supported_files(
        paths: list[pathlib.Path], supported_extensions: Set[str], include_hidden: bool = False
    ) -> list[pathlib.Path]:
        ...
        for path in paths:
            if (
                path.is_file()
                and FileTypeUtils.is_supported_extension(path, supported_extensions)
                and (include_hidden or not FileSystemUtils.is_hidden_file(path))
            ):
...
class TempFileUtils:
    ...
    def create_temp_file(
        suffix: str = "", prefix: str = "mistral_ocr_"
...
class PathUtils:
    ...
    def resolve_with_fallback(
        primary_path: Optional[Union[str, pathlib.Path]], fallback_path: pathlib.Path
    ) -> pathlib.Path:
    ...
    def generate_unique_filename(
        base_path: pathlib.Path, prefix: str = "", suffix: str = ""
    ) -> pathlib.Path:
